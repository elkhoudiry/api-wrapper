...
test('query builder integrity: no properties', async () => {
    ...
    let test_column: SqlColumn = { name: 'id', type: 'INTEGER', notNull: false, primary: false, autoGenerate: false };
...
test('query builder integrity: not null', async () => {
    ...
    let test_column: SqlColumn = { name: 'name', type: 'TEXT', notNull: true, primary: false, autoGenerate: false };
    let postfix: CreateTablePostfix = { columns: [test_column] };
...
test('query builder integrity: primary key', async () => {
    ...
    let test_column: SqlColumn = { name: 'name', type: 'TEXT', notNull: false, primary: true, autoGenerate: false };
    let postfix: CreateTablePostfix = { columns: [test_column] };
...
test('query builder integrity: auto generate as identity', async () => {
    ...
    let test_column: SqlColumn = { name: 'name', type: 'TEXT', notNull: false, primary: false, autoGenerate: true };
    let postfix: CreateTablePostfix = { columns: [test_column] };
...
test('query builder integrity: default value', async () => {
    ...
    let test_column: SqlColumn = { name: 'name', type: 'TEXT', notNull: false, primary: false, autoGenerate: false, default: 'ahmed' };
    let postfix: CreateTablePostfix = { columns: [test_column] };
...
test('create new table with 1 column at least should success', async () => {
    const id_column: SqlColumn = { name: 'id', type: 'INTEGER', notNull: false, primary: true, autoGenerate: true };
